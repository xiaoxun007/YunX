...
> 发布物说明：本 fork 的 Releases 产物为 **Release 构建**（R8 混淆 + 资源压缩，体积约 4MB，使用随仓库公开的固定 release keystore 签名，可直接安装、跨版本覆盖升级）；`assembleRelease` 即可复现。签名密钥见 `app/release.keystore` + `app/keystore.properties`，为 fork 分发专用，**勿用于官方发布**。
